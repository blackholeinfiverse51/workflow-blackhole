...
// Get single employee hourly rate
router.get('/employee-hourly-rate/:userId', auth, async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    res.json({
      ...
      data: {
        ...
        salaryType: employee.salaryType,
        calculatedHourlyRate: employee.calculatedHourlyRate,
        standardShiftHours: employee.standardShiftHours,
        standardWorkingDays: employee.standardWorkingDays
      }
    });
  } catch (error) {
    console.error('❌ Get employee hourly rate error:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.get('/daily', auth, async (req, res) => {
  try {
    ...
    if (date) {
      // Parse the date and set to start/end of day in Asia/Kolkata timezone
    ...
    if (departmentId) {
      records = records.filter(r => 
        r.user?.department?._id?.toString() === departmentId
      );
    }
